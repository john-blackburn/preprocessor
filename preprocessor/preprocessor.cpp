...
void processFile(string fname, map<string,Var> &vars, const list<string> &includePaths, list<string> &included)
{
    ...
    if (g_debug) fprintf(g_outfile, "# %d \"%s\" 1\n", g_lineno, fname.c_str());
    ...
    list<string> &included, map<string, string> attributeMap, bool hasIncludeSupported)
{
    ...
    // ----------------------------------------------------------------------
    // replace backslashes in fname with forward slashes
    // replace backslashes in includepaths with forward slashes
    // and ensure each includepath ends in /
    // ----------------------------------------------------------------------
...
bool checkAgainstMSVC(char *code, char *MSVCcode, string file, string MSVCfile)
{
    const char* sep = "=================================================================================";
    ...
    while (true)
    {
        pptok = getTok(stpp, &edpp, &skipped);

        // Skip LINE statements in .i file
        while (true)
        {
            itok = getTok(sti, &edi, &skipped);
            sti = edi;

            if (itok.type != LINE) break;

            getTok(sti, &edi, &skipped);
            sti = edi;
            getTok(sti, &edi, &skipped);
            sti = edi;
        }

        if (pptok.type == -1 && itok.type == -1) break;
        if (pptok.type == -1)
        {
            printf("%s\n", sep);
            printf("FAIL: %s shorter than %s\n", file.c_str(), MSVCfile.c_str());
            printf("%s\n", sep);
            return false;
        }
        if (itok.type == -1)
        {
            printf("%s\n", sep);
            printf("FAIL: %s shorter than %s\n", MSVCfile.c_str(), file.c_str());
            printf("%s\n", sep);
            return false;
        }
        if (pptok.type != itok.type || pptok.id != itok.id)
        {
            printf("%s\n", sep);
            printf("ERROR: Files differ at line %d in %s:\n", lineno, file.c_str());
            printf("%s: type=%s id=\"%s\"\n", file.c_str(), names[pptok.type], pptok.id.c_str());
            printf("%s: type=%s id=\"%s\"\n", MSVCfile.c_str(), names[itok.type], itok.id.c_str());
            printf("%s\n", sep);

            if (pptok.type == STRING && itok.type == STRING)
                printf("Only a difference in STRING. Might not be serious. Continuing...\n");
            else if (pptok.type == WSTRING && itok.type == WSTRING)
                printf("Only a difference in WSTRING. Might not be serious. Continuing...\n");
            else
                return false;
        }

        for (char* p = stpp; p < edpp; p++)
        {
            if (*p == '\n') lineno++;
        }
        stpp = edpp;
    ...
    printf("%s\n", sep);
    printf("PASS: Files %s and %s are identical (ignoring whitespace differences)\n", file.c_str(), MSVCfile.c_str());
    printf("%s\n", sep);
